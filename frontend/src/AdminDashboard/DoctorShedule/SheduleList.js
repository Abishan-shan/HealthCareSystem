...
const SheduleList = () => {
  ...
  const onDelete = async (id) => {
    ...
    const response = await fetch(`http://127.0.0.1:8001/api/Schdelete/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const onEdit = async (id) => {
    ...
    const response = await fetch(`http://127.0.0.1:8001/api/Schedit/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const onSubmit =async (e) => {
    e.preventDefault();
    Udata.day=Udata.day.toString();
    console.log(JSON.stringify(Udata));
      
    
    
    const response = await fetch("http://127.0.0.1:8001/api/Schupdate/"+id,{
  ...
  return (
    <>
      <Row className="mt-1 row">
        ...
        {/* this is start from edit */}

        {showEdit && (
          <Col xl={9} sm={7} xxs={10} xs={6} lg={9} md={8} className=" scroll">
            <Card className="colScroll  mb-5 ms-5">
